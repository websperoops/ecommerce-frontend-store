...
const Dashboard = ({ title, description, children }) => {
  ...
  const { pending, processing, delivered } = useFilter(data ? data : []);
  ...
  useEffect(() => {
    ...
    //if (data.length > 0) {
    //  let totalCoins = data.reduce(prev, current => {
    //    return prev + Number(current.coins);
    //  }, 0)
    //  setCoins(totalCoins)
    //}
  ...
  return (
    ...
    >
      {userInfo && <div className="mx-auto max-w-screen-2xl px-3 sm:px-10">
